#######################################################
###### 1.3_Rice_HGPAG_Summary_statistics_of_GWAS ######
#######################################################
...
SS_gwas <- function(res, x, map.x, qtn.candidate, gene.set = NULL, n.top.false.block = 10, 
  ...
  if(is.null(gene.set)){
    ...
    while(length(jun) >= 1){
      ...
      if(check.true){
        ...
        if(any(is.na(match(qtn.candidate2, qtn.candidate2s)))){
          count1 <- count1 + sapply(cand.delete, function(x) ifelse(length(x) >= 1, 1, 0))
          count2 <- count2 + 1
    ...
    qtn.logp.LD.order.seg <- order(unique(max.trues), decreasing = TRUE)
    len.qtn.LD <- length(qtn.logp.LD.order.seg)
    
    if(len.qtn.LD == count2){
      count.FN <- table(max.trues)[rev(qtn.logp.LD.order.seg)]
      names(count.FN) <- NULL
      TP.each <- sapply(count1, function(x){
        count1.0 <- rep(0, len.qtn.LD)
        count1.0[!is.na(match(qtn.logp.LD.order.seg, 0:x))] <- count.FN[!is.na(match(qtn.logp.LD.order.seg, 0:x))]
        return(count1.0)
      })
    }else{
      if(len.qtn.LD < count2){
        ...
        names(count.FN) <- NULL
        TP.each <- sapply(count1, function(x){
          count1.0 <- tapply(!is.na(match(order(max.trues, decreasing = TRUE), 0:x)),
                             INDEX = max.trues, sum, na.rm = TRUE)
          
          count1.0 <- count1.0[match(round(unique(max.trues), 3), round(as.numeric(names(count1.0)), 3))]
          return(count1.0)
        })
      }else{ 
        count1.plus <- len.qtn.LD - count2 + count1
        TP.each <- sapply(count1.plus, function(x){
          count1.0 <- tapply(!is.na(match(order(max.trues, decreasing = TRUE), 0:x)),
                             INDEX = max.trues, sum, na.rm = TRUE)
        ...
        TP.each <- rbind(apply(TP.each[1:2, ], 2, sum), TP.each[3, ])
        len.qtn.LD <- count2
        count.FN <- c(2, 1)
      }
    }
    rownames(TP.each) <- paste0("causal_", 1:len.qtn.LD)
    FP.each <- matrix(rep(count3, len.qtn.LD), nrow = len.qtn.LD, byrow = TRUE)
    FN.each <- matrix(rep(count.FN, ncol(TP.each)), ncol = ncol(TP.each)) - TP.each
    TN.each <- count0 - TP.each - FP.each - FN.each
    
    
    FDR.each <- FP.each / (FP.each + TP.each)
    FPR.each <- FP.each / (FP.each + TN.each)
    FNR.each <- FN.each / (TP.each + FN.each)
    Recall.each <- TP.each / (TP.each + FN.each)
    Precision.each <- TP.each / (TP.each + FP.each)
    Accuracy.each <- (TP.each + TN.each) / (TP.each + FP.each + TN.each + FN.each)
    Hm.each <- 2 * Recall.each * Precision.each / (Recall.each + Precision.each) 
    
    if(any(is.nan(Precision.each))){
      FDR.each[is.nan(Precision.each)] <- NA
      Hm.each[is.nan(Precision.each)] <- NA
      Precision.each[is.nan(Precision.each)] <- NA
    }
    
    if(any(Recall.each == 0 | Precision.each == 0)){
      Hm.each[Recall.each == 0 | Precision.each == 0] <- 0
    }
  ...
  return(list(log.p = log.p, qtn.logp.order = qtn.logp.order.seg, thres = thress, overthres = overthress,
              ...
              Hm.other = Hm.other, FDR.each = FDR.each, FPR.each = FPR.each, FNR.each = FNR.each,
              Recall.each = Recall.each, Precision.each = Precision.each, Accuracy.each = Accuracy.each, 
              Hm.each = Hm.each, haplo.name = haplo.name, mean.false = mean.false, max.trues = max.trues,
...
AUC_causal <- function(res, x, map.x, qtn.candidate, LD_length = 15000, cor.thres = 0.35,
                       window.size = 0, gene.set = NULL){
  map.x <- map.x[order(map.x[, 2], map.x[, 3]), ]
  marker <- as.character(map.x[, 1])
  chr <- map.x[, 2]
  pos <- map.x[, 3]
  ...
  if(is.null(gene.set)){
    ...
    cand.arounds <- NULL
    for(i in 1:n.causal){
      cand.around0 <- which(cum.pos >= (cum.pos[qtn.candidate[i]] - LD_length) & 
                              cum.pos <= (cum.pos[qtn.candidate[i]] + LD_length)) 
      
      
      cor.now <- cor(x[, qtn.candidate[i]], x[, cand.around0]) ^ 2
    ...
    match.qtn.candidate <- match(qtn.candidate[1:n.causal], cand.around)
  }else{
    n.causal <- 1
    ...
    cand.nows <- gene.set[which(gene.names == gene.name[haplo.no[1:n.causal]]), 2]
    ...
    cand.around <- cand.haplo.around
    match.qtn.candidate <- match(haplo.no[1:n.causal], cand.around)
  }
  
  
  
  pvals.LD.block <- res[cand.around, 4]
  ord.pvals.LD.block <- rank(-pvals.LD.block)
  ord.qtn.candidate <- ord.pvals.LD.block[match.qtn.candidate]
  
  
  AUC.causal.TP <- c(0, 1:n.causal, n.causal) / n.causal
  AUC.causal.FP <- (c(0, sort(ord.qtn.candidate), length(cand.around)) - c(0, 1:n.causal, n.causal)) / (length(cand.around) - n.causal)
  AUC.causal.FP[AUC.causal.FP <= 0] <- 0
  
  
  AUC.causal <- AUC.causal.TP[2] * AUC.causal.FP[2] / 2
  
  for(m in 2:(length(AUC.causal.FP) - 1)){
    AUC.causal.segment <- (AUC.causal.TP[m] + AUC.causal.TP[m + 1]) * (AUC.causal.FP[m + 1] - AUC.causal.FP[m]) / 2
    AUC.causal <- AUC.causal + AUC.causal.segment
  }
